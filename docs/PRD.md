...
- Above the textarea:
  ...
  - **Presenters** section, always visible above the textarea: "How many 1 2 3" (default 1, or the number of `Name:` prefixes found), then one row per presenter with an optional name (placeholder "You" for one presenter, "Presenter 1", "Presenter 2" for more) and a voice picker with a play button for the pre-rendered sample. Picking 2 or 3 for a script without prefixes shares the slides in turn by adding a name to the first line of each slide. Renaming a presenter renames its `Name:` prefixes at line starts when the name is committed (Enter or leaving the field), so a half-typed name never reaches the script. A name that reads as a slide line ("Slide 2") is refused. Picking 1 for a script with several presenters plays every line as presenter 1 and leaves the text as it is. Picking 2 when the script itself gives lines to 3 presenters changes nothing and is not saved; a note under the rows says whose lines to move, or to pick 1. A whole new text (Paste here, Ctrl+V or Cmd+V over the whole script, a text file, Draft from my slides) follows its own names, even names used once each when at least half of the lines carry one; a text without names is shared in turn between the presenters picked.
  - **Write it with an AI chat (free)** reveals the presenters it will ask for (from the section above), **Copy prompt**, links to ChatGPT, Claude and Gemini, and "Paste the answer here" (focuses the textarea). The prompt contains every slide's text, the presenter count and names, and the exact format, and asks for the script inside one code block. Under it, **Or let Presentice write it for you** takes a free Google Gemini API key (password field, kept in `localStorage` under `presentice:ai-key`, **Remove key** next to it), sends that same prompt to `generativelanguage.googleapis.com` with the key in the `x-goog-api-key` header (never in a URL), and replaces the script with the answer, with one undo step back. Copy prompt stays the default path for anyone without a key. Every failure has its own plain sentence: no key, refused key, rate or quota limit, offline, blocked by the browser, refusal, empty or unusable answer, no answer in time.
...
- Opened from Practice for later changes. Shows the same presenters and voices as the Script step; rename there. Colors are assigned automatically; each presenter gets a different default voice.
...
- **Voice quality** (user choice, never automatic): **Standard** = q8 on WASM (about 120 MB, about 121 MB in the tab, no GPU memory, about 2.5x slower than real time) is the default for everyone; **High** = fp32 on WebGPU (about 350 MB, about 2 GB in the tab plus 1.2 GB of GPU memory, faster than real time) is offered only when `navigator.gpu.requestAdapter()` returns an adapter, and is disabled with one line of copy otherwise. Measured on an 8 GB laptop with one 13 second line: q8/wasm loaded in 6.9 s and generated in 33 s; fp32/webgpu loaded in 21 s and generated in 8 s but froze the machine. fp16 on WebGPU (about 9 dB quieter, a third of the signal) and q8 on WebGPU (157 s for the same line, wrong duration) are never used. The worker loads exactly what it is asked for: a failure is reported, never silently swapped for another engine. The choice is saved with the project.
...
- **Audio format**: trimmed of leading and trailing silence (RMS threshold, about 80 ms padding kept), converted to 16-bit PCM WAV (48 KB/s), stored as ArrayBuffer plus duration. Cache key: SHA-256 of (normalized text, voice, dtype, device, model id), so edits, voice changes and a Voice quality switch regenerate only what changed and never serve mismatched audio. Pruning keeps the clips of the other quality for the sentences in the script, so switching back costs nothing.
...
- **Backend**: reversed after listening to samples on an 8 GB Mac. q8 on WASM is now the default for everyone, because fp32 on WebGPU peaks at about 2 GB in the renderer plus 1.2 GB of GPU memory and freezes that machine, while q8 sounds good enough to approve. Speed is the price, and the status line states it: an estimate computed from this engine's measured throughput, not from a guess. WebGPU is still there as **High**, but only when the user asks for it.
- **Steps**: merged Script and Review into one screen (textarea plus read-only preview) and turned Presenters into an optional panel. Presenter count and voices now sit in a Presenters section on the Script step, right after upload; the AI chat option reuses them. Presenters still come from `Name:` prefixes, and the section writes those prefixes for you.
...
- **Audio cache key**: adding Voice quality put the device into the key (`text, voice, dtype, device, model`), which orphaned every clip the released version had saved. Making a prepared deck again on Standard, at about 2.5 times slower than real time, is not an acceptable upgrade, so `js/tts.js` renames a pre-upgrade clip the first time that sentence is needed again. The old format stood for q8 on WASM or fp32 on WebGPU, depending on the machine, which are Standard and High today, so each clip is carried over to the key of the quality that made it, whichever quality is selected at the time. The prune keeps a pre-upgrade clip until its carry-over has run, so the sweep can never destroy what the carry-over exists to save; clips of sentences the script no longer has are swept as usual.
